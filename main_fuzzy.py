    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        if args.train:
            ...
            for batch_idx, sample in enumerate(TrainImgLoader):
                ...
                if args.train_deeplab:
                    sig_loss,sig_only_loss,epe_loss,epe, scalar_outputs, image_outputs=sig_sample(sample,compute_metrics=do_summary)
                ...
                if args.train_deeplab:
                    if (batch_idx%100==1)or(batch_idx==len(TrainImgLoader)-1):
                        ...
                        print('Epoch {}/{}, Iter {}/{},sig_loss = {:.3f},sig_pixel = {:.3f},origin_epe = {:.3f},refined_epe={:.3f}, time = {:.3f},sig loss avg={:.3f}'.format(epoch_idx, args.epochs,
                                                                                        batch_idx,
                                                                                        len(TrainImgLoader), sig_loss,sig_only_loss,epe_loss,epe,
            ...
            if (epoch_idx + 1) % args.save_freq == 0:
                ...
                if args.train_deeplab:
                    ...
                    torch.save(checkpoint_data, "{}/sig_checkpoint_fuzzy_{:0>6}.ckpt".format(args.logdir, epoch_idx))
            gc.collect()
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            start_time = time.time()
            do_summary = global_step % args.summary_freq == 0
            epe_origin,epe,epe_refine, sig_loss, scalar_outputs, image_outputs = test_sample(batch_idx,sample, compute_metrics=do_summary)
            ...
            if (batch_idx%100==0)or(batch_idx==len(TestImgLoader)-1):
                print('Epoch {}/{}, Iter {}/{},origin epe={:.3f},epe={:.3f},refine epe={:.3f},test sig loss={:.3f}, time = {:3f}'.format(epoch_idx, args.epochs,
                                                                                     batch_idx,
                                                                                     len(TestImgLoader),epe_origin,epe,epe_refine,sig_loss,
                                                                                     time.time() - start_time))
            #print(avg_test_scalars["EPE"])
            if epe<epe_origin:
                good_sample+=1
        print('the good sample is {}/{}'.format(good_sample,batch_idx))
...
# train one sample
def sig_sample(sample, compute_metrics=False):
    model.eval()
    mono_model.eval()
    deeplab_model.train()
    ...
    sig_gt=torch.cat([(mono-bio).unsqueeze(1),(bio-mono).unsqueeze(1)],dim=1).float()
    sig_gt=torch.sigmoid(sig_gt)
    # mbb=((bio-mono)>0).float()#m比b好为1
    # bbm=((mono-bio)>0).float()
    # # target=mbb#mbb为1
    # target=((bio-mono)>args.sig_arg).float()
    ...
    sig_output=sig_output.softmax(1)
    ...
    # print(sig_output.shape,sig_gt.shape)
    sig_loss=fuzzy_cross_entropy(sig_output,sig_gt)
    ...
    epe_loss = F.smooth_l1_loss(disp_est_pred_refine[mask], disp_est_refine[mask], size_average=True)*2
    scalar_outputs["loss EPE"] = [epe_loss]
    loss_weight=0.85
    all_loss=loss_weight*sig_loss+(1-loss_weight)*epe_loss
    all_loss.backward()
    deeplab_optimizer.step()
    return  tensor2float(all_loss),tensor2float(sig_pixel),tensor2float(origin_epe),tensor2float(refined_epe),tensor2float(scalar_outputs), image_outputs
...
def test_sample(batch_idx,sample, compute_metrics=True):
    ...
    disp_ests,confidence,confidence_var,index = model(imgL, imgR)
    disp_ests_final= disp_ests[-1]
    disp_est_origin=disp_ests_final
    ...
    disp_est_pred_refine= disp_est_pred_unocc+mono_disp_est_pred_occ
    disp_ests[-1]=disp_est_pred_refine
    epe=EPE_metric(disp_ests[-1], disp_gt, mask)
    d1=D1_metric(disp_ests[-1], disp_gt, mask)
    #loss = model_loss(disp_ests, disp_gt,mask,imgL, imgR)
    ...
    scalar_outputs["origin EPE"] = [EPE_metric(disp_est_origin, disp_gt, mask)]
    image_outputs["mono errormap"] = [disp_error_image_func()(mono_disp_est, disp_gt)]
    scalar_outputs["mono EPE"] = [EPE_metric(mono_disp_est, disp_gt, mask)]
    ...
    scalar_outputs["D1_refine_gt"] = [D1_metric(disp_est_refine, disp_gt, mask)]
    scalar_outputs["D3_refine_gt"] = [D3_metric(disp_est_refine, disp_gt, mask)]
    ...
    return tensor2float(origin_epe),tensor2float(epe),tensor2float(gt_epe), tensor2float(sig_loss), tensor2float(scalar_outputs), image_outputs
#改返回值～～～主函数输出
